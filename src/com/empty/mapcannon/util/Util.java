...
public class Util {
    ...
    public static int getAppVersionCode(Context paramContext) {
        try {
            int i = paramContext.getPackageManager().getPackageInfo("cn.thumbworld.leihaowu",
                    0).versionCode;
            return i;
        } catch (Exception localException) {
            localException.printStackTrace();
    ...
    public static long getLastCallDuration(Activity paramActivity, String paramString) {
        ...
        Cursor localCursor = paramActivity.getContentResolver().query(CallLog.Calls.CONTENT_URI,
                new String[] {
                        "duration", "type", "date"
        }, "number=?", new String[] {
                "10086"
    ...
    public static Drawable getPic(Context paramContext, String paramString) {
        ...
        return (BitmapDrawable) Drawable
                .createFromPath(new File(paramContext.getCacheDir(), str).toString());
    }
    ...
    public static void setListViewHeightBasedOnChildren(ListView paramListView) {
        ...
        localLayoutParams.height = (i
                + paramListView.getDividerHeight() * (-1 + localListAdapter.getCount()));
    ...
    public boolean delAllFile(String paramString) {
        ...
        if (i < arrayOfString.length) {
            if (paramString.endsWith(File.separator))
                ;
            for (File localFile2 = new File(paramString + arrayOfString[i]);; localFile2 = new File(
                    paramString + File.separator + arrayOfString[i])) {
